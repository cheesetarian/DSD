...
def create(address, run, n_dimen):
    ...
    print('columns reduced: ',col_reduced)
...
def apply(address, run):
    ...
    # Print X_pca to file
    Print.printPCAToFile(address, run, lon, lat, X_pca, col_reduced)  
...
def PrincipleComponentAnalysis( address, run, X_train, n_dimen):
    ...
    #X_pca_train = X_train #pca.transform(X_train)    #  just avoid using PCA here?????????
    ...
    del pcaObject    
    print("PCA variance explained (%): ",str(np.round(100*pca.explained_variance_ratio_, 1)))
